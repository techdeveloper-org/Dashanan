...
`[FUTURE ADR CANDIDATE, flagged 2026-09-17, not a blocker for Phase 1: Adaptive Weight Tuning -- w1..w6 should eventually stop being static per-deployment constants and instead be learned/adjusted per workload type (e.g. a long-running coding agent plausibly needs higher w3 Importance + w5 TaskRelevance + w6 ProvenanceConfidence and lower w1 Recency + w2 Frequency than a short-lived customer-support session, matching the reviewer's own intuition that Recency/Frequency are currently over-weighted relative to Importance/TaskRelevance/ProvenanceConfidence for most real deployments). Candidate approaches to evaluate at that ADR's time: online gradient-based tuning against a retrieval-quality objective, a small per-workload-type preset library, or periodic offline re-fitting against logged promotion/archive outcomes. MVP ships with the static equal-weighted default above; this ADR is explicitly deferred, not designed here.]`

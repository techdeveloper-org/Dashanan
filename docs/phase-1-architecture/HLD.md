...
ADR-017: Write-gate regulated-identifier scrub
  Status:    PROPOSED (PARTIALLY signed off)  [narrows ADR-014's log-tier filter;
             consensus-gate sign-off 2026-09-18, OAQ-18 PARTIALLY closed]
  ...
  Sign-off:  OAQ-18 PARTIALLY closed. (1) Identifier-set recommendation (india-cyber-
      compliance-architect persona, citation-backed, NOT legal confirmation): confirm
      Aadhaar (Aadhaar Act 2016 Section 29 -- bars sharing/use of Aadhaar identity
      information beyond authorized purposes, a standalone statutory basis independent
      of DPDP), PAN (IT Act 2000 Section 43A + SPDI Rules 2011's "financial
      information" limb -- the weaker of the three citations, financial-identifier
      proximity rather than a direct statutory "PAN is sensitive" clause), and payment
      card numbers (SPDI Rules 2011's direct enumeration of card/payment-instrument
      details). Correction for the record: DPDP Act 2023 as ENACTED removed the
      sensitive-personal-data/critical-personal-data tiering present in the 2019/2021
      Bill drafts and in GDPR -- do not cite a DPDP-specific "sensitive category" basis
      for these identifiers in any regulator-facing document; ground elevated
      treatment in Aadhaar Act Section 29 and IT Act Section 43A/SPDI Rules instead.
      Considered and excluded from this narrow (non-redaction) detector's default set:
      bank account/IFSC numbers, passport/voter-ID/driving-licence numbers -- left to
      per-deployment config extension rather than hardcoded, consistent with this
      ADR's existing extensibility design. All of the above requires counsel
      ratification before use in any regulator-facing document; it is a recommendation,
      not a closure of the legal-confirmation sub-question. (2) Fail-open vs.
      fail-closed: a reasoned recommendation (fail-closed -- 503 + Retry-After, no
      grace period) is recorded here as input for whoever runs the actual gate: the
      asymmetry of harm favors fail-closed (a fail-open leak persists a regulated
      identifier in the clear, retrievable via Zone 6 similarity search per threat
      I-6a, remediable only via DPDP-2's P2/review-only crypto-shredding cascade,
      versus a rejected write costing only a retry); the detector sits before the
      ADR-010 journal fsync, structurally the same class of dependency as the
      structured store (Section 8.4, which also fails closed), not a degradable
      read-path dependency; no grace period is warranted since the detector carries no
      Section 8.4 failure-mode row of its own (in-process logic, not a networked
      dependency). **This recommendation is NOT adopted at this gate** -- per this
      ADR's own Consequences text above, the fail-open/fail-closed choice is explicitly
      deferred to Phase 1.5 API-contract sign-off, a different, later gate than this
      HLD's Phase-1 consensus gate, and this sign-off pass does not constitute that
      gate. Status remains PROPOSED for this specific sub-question.
```
...
Decision:  Replace the single-connection factory with psycopg_pool.ConnectionPool (sync, matching
  ...
  Full design: docs/phase-1.5-design/connection-pooling-design.md (v2.6, DRAFT -- one disclosed
  ...
  Consequences:
    ...
    Documented exception (added 2026-09-22, solution-architect review finding 2; CLOSED 2026-09-22,
      user decision -- see connection-pooling-design.md v2.6 Change Log): Section 6's
      ...
      chose to close it by REQUIRING the breaker. connection-pooling-design.md v2.6 Section 5.1
  ...
  Sign-off:  NOT YET SIGNED OFF. This ADR entry documents a Sprint 5 planning-stage proposal
      (docs/phase-1.5-design/connection-pooling-design.md v2.6, DRAFT), not an architect-approved,
...
Decision:  A dedicated entity_schema.sql (two tables: entity_attributes, row-per-attribute
    ...
    UnifiedSubjectErasureOrchestrator's existing erase_entity call site and behavior need zero
    changes when the composition root's Zone 5 builder is swapped from Shape A to Shape B --
    CORRECTED 2026-09-23, consensus-agent failure-mode-review finding 2: the orchestrator's
    constructor DOES require one small, non-behavioral type-annotation widening, zone5_repository:
    EntityMemoryRepository -> a new local Protocol exposing only erase_entity, so mypy strict
    accepts a SqlEntityMemoryRepository instance at that call site; see design doc Section 8 item 1
    v4 for the full finding and rollback path), and an extension of
  ...
  Full design: docs/phase-1.5-design/zone5-shape-b-storage-design.md (v4, DRAFT -- 5 disclosed
...
| OAQ-4 | **NEW REQUIREMENT:** per-zone **capacity cap + MaxAge forced archival** | **Resolved: Adopted** (consensus-gate sign-off, 2026-09-18) | Not in the PRD. Without it, high-`I+P` items are pinned in their zone forever (OAQ-3) and zones grow without bound. This converts the Memory Score from a pure threshold function into **a ranking function with a threshold fast-path**, which is the only formulation that guarantees liveness. Affects FR-012's scope. Sign-off confirmed the requirement is sound (§12F already names it the sole Sprint-1 growth backstop) and that **per-zone** caps are correct over a single global cap, since §12A's own per-zone table already shows orders-of-magnitude size differences across zones (Zone 1: 200/session vs. Zone 8: 5,000,000) that a global cap couldn't respect — consistent with OAQ-16's finding that Zones 6/7 are cross-cutting derived layers over the other six, not that all zones share one undifferentiated capacity profile. Concrete cap/MaxAge values are already set in §12A with half-life derivations; this sign-off is mechanism approval only, not new numbers. |
...
| OAQ-10 | DPDP erasure via **crypto-shredding** of an append-only audit store | Proposed, with cited research context (added 2026-09-20, corrected 2026-09-20 per external review of source quality) — **still requires legal confirmation, status unchanged** | Resolves a genuine legal/technical conflict, but whether key destruction constitutes erasure under DPDP Act 2023 requires legal confirmation, not an architect's judgment. **Real research context (not a legal opinion), primary sources first:** (1) **Primary source:** [THE DIGITAL PERSONAL DATA PROTECTION ACT, 2023 — official Act text, meity.gov.in](https://www.meity.gov.in/static/uploads/2024/06/2bf1f0e9f04e6fb4f8fef35e82c42aa5.pdf), Section 12. Supporting secondary interpretation: [DPDPA.com, Section 12 with interpretation](https://www.dpdpa.com/dpdpa2023/chapter-3/section12.html). Section 12's erasure right is *conditional and narrower than GDPR's right to be forgotten* — anchored to two specific triggers (consent withdrawal, or cessation of the purpose data was collected for), and a Data Fiduciary may refuse erasure if retention is required by law (e.g. audit/tax records). This matters for Zone 7 specifically: an append-only *audit* record is plausibly retained-for-compliance under this exception, which is a different legal question from whether crypto-shredding *counts as* erasure at all. (2) **Corrected claim on international precedent (the original 2026-09-20 draft overstated this — flagged by external review, fixed against the primary EU regulator source):** crypto-shredding's sufficiency under GDPR Article 17 is **not a settled, accepted mechanism** — the EU's own **EDPB** (European Data Protection Board, the primary authority, not a secondary blog) states directly that "whether encryption combined with key destruction is considered effective deletion within the meaning of Article 17 GDPR remains under consideration" [EDPB, Coordinated Enforcement Action — implementation of the right to erasure by controllers, edpb.europa.eu](https://www.edpb.europa.eu/our-work-tools/our-documents/other/coordinated-enforcement-action-implementation-right-erasure_en), and EDPB Guidelines 05/2019 require erasure to be "verifiable and irreversible," extending to all storage layers including backups. The earlier draft's claim that "France's CNIL has acknowledged" this approach came from secondary industry sources (blog posts, not a direct CNIL/EDPB citation) and has been removed rather than kept unverified — supporting industry commentary describing crypto-shredding as a *practical* approach remains available for context: [Conduktor, "Crypto Shredding for Kafka"](https://www.conduktor.io/glossary/crypto-shredding-for-kafka), [VeritasChain, "Crypto-Shredding: GDPR + MiFID II Compliance"](https://veritaschain.org/blog/posts/2026-01-18-crypto-shredding-gdpr-mifid-ii-reconciliation/) — but these are industry commentary, not regulatory authority, and are cited here only as supporting context, never as the basis for a compliance claim. **Net effect of the correction: the international precedent for crypto-shredding is weaker than first drafted — even the EU's own equivalent question is unsettled — which if anything argues for MORE caution before relying on it for DPDP, not less.** No Indian regulatory guidance (MeitY or otherwise) endorsing crypto-shredding specifically under DPDP was found. **This upgrades the OAQ from "no research" to "informed, with primary-sourced citations" — it does not close it; real counsel confirmation is still required before this can be adopted.** |
...
| OAQ-18 | **Proposed:** regulated-identifier detection at the write gate (ADR-017) | **Partially resolved** (consensus-gate sign-off, 2026-09-18); **additional research context added 2026-09-20, status unchanged** | Narrows ADR-014's log-tier-only PII filter to also cover a small, explicit class of regulated structured identifiers in persisted/indexed content — deliberately not general redaction (see ADR-017 Why §3 and its interaction with DPDP-1/DPDP-2). Sign-off produced a citation-backed identifier-set recommendation (Aadhaar — Aadhaar Act 2016 §29; PAN and payment card numbers — IT Act 2000 §43A + SPDI Rules 2011; explicitly correcting a common but unverified claim that DPDP Act 2023 itself has a "sensitive personal data" tier — the enacted Act removed that tiering) — **framed as a recommendation pending counsel ratification, not legal confirmation**. The fail-open/fail-closed posture question is recorded as a reasoned recommendation (fail-closed) for whoever runs the actual gate, but per ADR-017's own Consequences text this specific choice is explicitly deferred to Phase 1.5 API-contract sign-off, not this Phase-1 HLD gate — so it is NOT marked adopted here, only informed. Both (a) legal ratification of the identifier set and (b) the formal Phase 1.5 fail-open/closed decision remain explicitly open. **Real research context added 2026-09-20, corrected 2026-09-20 per external review (primary sources promoted, one claim qualified rather than overstated):** (1) **Primary source:** [UIDAI Circular No. 8 of 2025 (dated 18/07/2025), Aadhaar Data Vault/HSM guidelines, uidai.gov.in](https://uidai.gov.in/en/ecosystem/authentication-devices-documents/authentication-document/19311-circular-8-of-2025-dated-18-7-2025-regarding-revised-guidelines-on-adv-hsm.html) and [UIDAI, "What is Masked Aadhaar?", uidai.gov.in](https://www.uidai.gov.in/en/283-faqs/aadhaar-online-services/e-aadhaar/1887-what-is-masked-aadhaar.html) together establish, on UIDAI's own primary text, that raw Aadhaar numbers must never be stored in open databases — any entity storing Aadhaar numbers/UID tokens/eKYC data must use a UIDAI-compliant Aadhaar Data Vault, and masked Aadhaar shows only the last 4 digits. This is a real, specific, non-discretionary storage prohibition (not just a masking preference), which argues for fail-closed being the materially safer default for any Zone touching Aadhaar-shaped identifiers, consistent with the existing recommendation. **Qualified rather than overstated (per external review):** the specific "FIPS 140-2 Level 3" HSM-certification detail was found only in secondary summaries of Circular 8/2025 ([Entrust, "UIDAI's Aadhaar Number Regulation"](https://www.entrust.com/digital-security/hsm/solutions/compliance/apac/uidais-aadhaar-number-regulation-compliance), [Lexology, "Vaulting to Safety"](https://www.lexology.com/library/detail.aspx?g=391c2c88-0ab3-4d29-b88d-1a53c68e45c9)), not independently confirmed from the primary circular's own clause text in this research pass — the binding claim this HLD relies on is narrowed to "a UIDAI-compliant Aadhaar Data Vault with certified HSM-based key management is required," not the specific FIPS certification level, until that clause is independently verified against the primary circular. (2) **Primary sources (verified accessible 2026-09-20, both directly fetched and confirmed genuine, not just search snippets):** [RBI Notification RBI/2022-23/77, "Restriction on Storage of Actual Card Data," rbi.org.in](https://www.rbi.org.in/scripts/NotificationUser.aspx?Id=12345) — the storage-restriction circular itself, addressed to all Payment System Providers/Participants, signed by P. Vasudevan (Chief General Manager) — and [RBI Notification RBI/2021-22/96, "Permitting Card-on-File Tokenisation (CoFT) Services," rbi.org.in](https://rbi.org.in/Scripts/NotificationUser.aspx?Id=12159&Mode=0), the earlier tokenisation-enablement circular this restriction builds on. Together RBI's own text confirms no entity in the card transaction/payment chain other than the card issuer/network may store actual card data (CoF data), with a limited exception for last-four-digits/issuer-name for reconciliation. Secondary summaries retained only as supporting plain-language context: [law.asia, "Card-on-file tokenisation introduced by RBI"](https://law.asia/card-on-file-tokenisation-rbi/). This reinforces that a missed payment-card identifier is a real regulatory-storage violation, not just a privacy risk, further supporting fail-closed. No new Indian regulatory source was found that changes the identifier set itself (Aadhaar/PAN/payment card) or that ratifies the recommendation as legally sufficient — counsel ratification remains the open item. |
| OAQ-19 | **Proposed:** UserAffinity session-aggregation method + cosine-mapping validity (§12G) | **Resolved: Adopted** (consensus-gate sign-off, 2026-09-18) | UserAffinity and TaskRelevance both had no formula anywhere in this HLD before §12G. Sign-off decided: (a) flat `mean(last N=5)` over an EWMA alternative — UserAffinity estimates a comparatively stable preference signal, not a decaying item value like Recency, so an EWMA would let one atypical session dominate; (b) `N=5` is defensible but made a per-tenant configurable on `/v1/config/scoring` rather than a hardcoded constant, so it can be revised from real data without a schema change; (c) the `(cosine+1)/2` mapping is **redesigned** to an adapter-calibrated min-max normalization (per-`model_id` empirical percentile range, calibrated at adapter registration and periodically thereafter — see §12G), applied identically to both UserAffinity and TaskRelevance, removing the unverified "every adapter has non-negative cosine" assumption rather than asserting it. |
| OAQ-20 | **Proposed:** Frequency `f_cap` per-zone default values (§12G) | **Partially resolved: assumed starting defaults** (consensus-gate sign-off, 2026-09-18) | The Frequency log-saturation formula (`log(1+access_count)/log(1+f_cap)`) is new in §12G; the per-zone `f_cap` values themselves are a tuning question analogous to §12A's threshold-tuning exercise. Sign-off proposed `[ASSUMED]` starting defaults for the two Sprint-1 zones that actually use Frequency (Zone 6/7 are derived/non-lifecycle per OAQ-16, no `f_cap` needed): Zone 1 (Working) `f_cap = 20`, Zone 2 (Episodic) `f_cap = 100`, both extrapolated from Section 9/12D's Profile B turn-rate proxy (`4 turns/min`, line 1772) — explicitly not measured or validated, same `[ASSUMED]` convention as NFR-004's targets. Locking real defaults from live access-count telemetry stays open, per this OAQ's own original text. |
...
**Note on OAQ-13, OAQ-20, OAQ-22 (added 2026-09-20):** unlike OAQ-10 and OAQ-18 above, these three
are **not web-researchable** and were deliberately not given a research pass in this round —
stated plainly here rather than left ambiguous. OAQ-13 (NFR-004 capacity/latency numbers) and
OAQ-22 (streaming-mandatory token threshold) require real load-testing/benchmarking against this
system's own actual running infrastructure, which doesn't exist yet (no Shape B deployment); a
web search can surface *industry-benchmark ranges for comparable systems* as general context, but
cannot produce Dashanan's own real measured numbers, and no such context was substituted here in
place of real measurement. OAQ-20 (per-zone `f_cap` defaults) requires real production access-count
telemetry specific to this system, which by definition doesn't exist before real usage — no
external source can substitute. All three remain exactly as before: `[ASSUMED]`/open, pending real
benchmarking or telemetry once Shape B infrastructure exists (see `backlog_draft.json`'s FR-015
stub, added 2026-09-20).
...
**UserAffinity** (consensus-gate sign-off, 2026-09-18 — OAQ-19 Resolved: Adopted):
```
UserAffinity = clamp01( (cosine(session_embedding, mean(last N=5 same-user prior session embeddings, tenant-scoped)) - cos_min[model_id]) / (cos_max[model_id] - cos_min[model_id]) )
```
Default `0.5` on cold start (no prior sessions for this user in this tenant). Intra-tenant only, per ADR-013 — never the isolation mechanism. Aggregation method decided: a flat mean over the last `N` sessions, not an exponentially-weighted alternative — UserAffinity estimates a comparatively stable user-preference signal, not a decaying item value like Recency (§12C), so an EWMA would let one atypical session (a one-off tangent, a shared account) dominate the term, which is the opposite of what it exists to estimate; applying Recency's decay shape here would be surface-level consistency, not architectural fit. `N=5` is retained as defensible but is now a **per-tenant configurable on `/v1/config/scoring`** (§7.3) rather than a hardcoded constant, so it can be revised from real access-pattern data without a schema change. The `(cosine+1)/2` mapping is replaced with an **adapter-calibrated min-max normalization** — see the calibration mechanism below — removing the previously unverified "every adapter has non-negative cosine" assumption rather than asserting it true.

**TaskRelevance** (consensus-gate sign-off, 2026-09-18 — OAQ-19 Resolved: Adopted):
```
TaskRelevance = clamp01( (cosine(item_embedding, query_embedding) - cos_min[model_id]) / (cos_max[model_id] - cos_min[model_id]) )
```
This is the formula OAQ-5 named as "raw normalized cosine" (as distinct from the RRF fused ranking score, ADR-008). It shares UserAffinity's cosine-mapping question and is updated identically — the two formulas must never diverge on this point, since a per-adapter calibration mismatch between them would silently skew one term's range relative to the other's.

**Adapter-calibrated cosine normalization (both formulas above):** each `EmbeddingProvider` adapter (ADR-015), on registration or first bind, runs a calibration pass over a fixed representative sample corpus (~500-1000 pairs drawn from the adapter's own embedding space, not tenant content — avoiding a DPDP calibration-data leak surface) and computes the empirical 1st/99th-percentile cosine similarity as `cos_min[model_id]` / `cos_max[model_id]` (percentile, not true min/max, to resist outlier skew). Stored keyed by `model_id`, consistent with ADR-015's existing per-vector `model_id` tracking — no new identity concept introduced. Re-calibration triggers: (1) on adapter version/checkpoint change (new `model_id`), (2) on a scheduled cadence (e.g. quarterly) to catch drift, (3) manually via the admin surface adjacent to `/v1/zones/{zone}/config`. `clamp01` still guards the final result. Degenerate-calibration fallback: if `cos_max[model_id] == cos_min[model_id]`, fall back to the original `(cosine+1)/2` mapping and log a warning rather than divide by zero.
...
| 1.5.6 | 2026-09-23 | solution-architect (consensus-agent round-3 review remediation on DASH-STORY-029) | `zone5-shape-b-storage-design.md` advanced v3->v4 (a consensus-agent failure-mode-review finding, round 6: the orchestrator's constructor requires a small type-annotation widening, narrowing the doc's own "zero orchestrator code changes" claim to "zero behavioral/call-site changes") -- ADR-021's Full-design citation and Decision paragraph, and Section 10's implementation-status note, were never updated to carry this correction, leaving HLD.md (this project's own architecture system-of-record) asserting the pre-correction blanket "zero code changes" claim after the design doc itself had already disclosed the real exception. Bumped the Full-design citation to v4, corrected the Decision paragraph and Section 10 note to state the narrowed claim with the type-annotation-widening exception. No other ADR content, STRIDE/DPDP finding, or OAQ was altered. |
| 1.5.7 | 2026-09-23 | solution-architect (consensus-agent round-5 review remediation on DASH-STORY-029) | ADR-020's own connection-pooling-design.md citations (Full-design line, Documented-exception paragraph, Section 6's design-patterns table row) were still at v2.4, one version behind that doc's real v2.6 (a Change-Log-table-ordering-only fix) -- bumped all four occurrences to v2.6. Non-blocking to DASH-STORY-029 itself (this is ADR-020's own citation, not ADR-021's), but caught during the same consensus-agent review pass as a sibling propagation miss and fixed in the same pass. No ADR content, STRIDE/DPDP finding, or OAQ was altered. |
| 1.5.8 | 2026-09-23 | solution-architect (consensus-agent round-6 review remediation on DASH-STORY-029) | Two more live (non-historical) stale citations found in the same "primary field fixed, sibling field missed" class: ADR-020's own Sign-off field still said "connection-pooling-design.md v1, DRAFT" three lines below its already-corrected Full-design line -- bumped to v2.6. Section 10's DPDP-2/DPDP-3 implementation-status note still cited dpdp-crypto-shredding-full-erasure-design.md as "v2.0, DRAFT, 10 documented solution-architect review rounds" (last touched at revision 1.5.1/round 8) even though the same paragraph's later sentence (correcting the zone5-shape-b-storage-design.md citation to v4) shows this paragraph is actively maintained -- bumped to "v2.3, DRAFT, 13 solution-architect review rounds plus a separate 4-round consensus-agent failure-mode/retry/rollback/escalation review". No ADR content, STRIDE/DPDP finding, or OAQ was altered. |
| 1.5.9 | 2026-09-23 | solution-architect (consensus-agent round-8 review remediation on DASH-STORY-029) | Revision History row 1.5.4 itself (the row that originally added ADR-021) still cited `zone5-shape-b-storage-design.md` as "(v3, DRAFT)" -- one more instance of the same "primary field fixed, sibling field missed" class rounds 1.5.5/1.5.6/1.5.8 already fixed elsewhere in this table and in ADR-021's own body text. Bumped row 1.5.4's citation to "(v4, DRAFT)". No ADR content, STRIDE/DPDP finding, or OAQ was altered. |

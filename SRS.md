...
| NFR-014 | Zone 1 concurrency control | The Zone 1 (Working Memory) structure SHALL use an explicit concurrency-control mechanism — Redis WATCH/MULTI-EXEC or an atomic Lua script on the per-session hash + sorted-set in Shape B; a per-session asyncio.Lock (or hash-striped lock table) in Shape A — so that concurrent multi-turn writes/reads against the same session cannot lose an update (ADR-018). On contention, the system SHALL retry a bounded number of times with backoff, then reject explicitly (503 + Retry-After) rather than block indefinitely on the synchronous write path. | High |
...
| AC-021 | NFR-014 | Given two concurrent tool calls write to the same session's Zone 1 structure, When both attempt a read-modify-write within the same window, Then the mechanism in ADR-018 ensures no update is silently lost, and if contention exceeds the configured retry budget the write is rejected with 503 + Retry-After rather than corrupting state. |
...
| 2026-09-18 | 1.0.3 | Close doc gaps from external architecture review (#3) | Added NFR-014/AC-021 for Zone 1 concurrency control (HLD ADR-018, new) — a real gap: ADR-005 had no concurrency-control policy for Zone 1's own hot buffer in either Shape A or Shape B, distinct from the SQL-based rotation-state CAS at §8.3. Added ADR-019 (new) for the `context.assemble` RPC transport shape (unary vs. streaming gRPC) at the 256k token-budget cap — a real but narrower gap than first claimed, since a cursor/pagination mechanism already existed. The other 2 claims in the same review (Vector DB partial-degradation contract, Phase 7 agent-handoff schema) were verified already correct/not-applicable; no SRS change needed for those. | Done |
| 2026-09-18 | 1.0.4 | Close verified documentation gaps: JSON schemas, RBAC matrix, async-worker RTM, DoD/dependency doc, DPDP retention policy | Added machine-readable JSON Schemas for the 3 representative orchestration handoff contracts, an RBAC matrix consolidating openapi.yaml's inline bearerAuth scopes, X-RateLimit-* response headers, an async-worker RTM in phase-2-validation, a consolidated Sprint 1 dependency-graph + DoD doc citing sp05-sa-review.md's C-2/C-3 corrections, and a DPDP retention-policy doc stating erasure is still P2/review-only pending AR1-G3. No FR/AC changes — consolidation of existing requirements only. | Done |
